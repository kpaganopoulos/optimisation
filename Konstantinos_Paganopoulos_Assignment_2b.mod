...
subject to constr1 {i in NUM}: x >= Chance[i] - (b0 + b1*GRE[i] + b2*TOEFL[i] + b3*Univ[i] + b4*SOP[i] + b5*LOR[i] + b6*CGPA[i] + b7*Res[i]);
subject to constr2 {i in NUM}: x >= (b0 + b1*GRE[i] + b2*TOEFL[i] + b3*Univ[i] + b4*SOP[i] + b5*LOR[i] + b6*CGPA[i] + b7*Res[i]) - Chance[i];